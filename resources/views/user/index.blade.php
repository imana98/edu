                      ...
                      <div class="relative h-200 w-200">
                        ...
                        @endif
                          <div class="flex-grow sm:text-left text-center mt-6 mb-4 p-2 sm:mt-0">
                            ...
                            <p class="leading-relaxed text-base">開催日：{{ $seminar->date }}<br>対象者：{{ $seminar->target }}<br>内容：{{ $seminar->explain }}</p>
                            @if ($seminar->deadline < $dd)
                            <div class="flex justify-end mb-8"></div>
                            @elseif (empty($user))
                            <div class="flex justify-end mb-8">
                              <button onclick="location.href='{{ route('user.seminars.show', ['id' => $seminar->id]) }}'" class="flex text-white bg-green-500 border-0 py-2 px-8 focus:outline-none hover:bg-green-600 rounded text-lg">登録する</button>
                            </div>
                            @endif
                          </div>
                      </div>
