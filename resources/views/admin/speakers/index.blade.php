            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="md:p-6 bg-white border-b border-gray-200">
                        <section class="text-gray-600 body-font">
                          <h3 class="text-3xl w-56 text-center mx-auto" style="background:linear-gradient(transparent 60%, rgb(179, 240, 167) 60%);">講義者管理</h3>
                          <div class="container md:px-5 mx-auto my-12">
                            <x-flash-message status="session('status')" />
                            <div class="flex justify-end mb-10">
                              <button onclick="location.href='{{ route('admin.speakers.create') }}'" class="flex text-white bg-green-500 border-0 py-2 px-8 focus:outline-none hover:bg-green-600 rounded text-lg">登録する</button>
                            </div>
                    ...
                    @foreach ($speakers as $speaker)
                            <tbody>
                              <tr>
                                <td class="px-4 py-3">{{ $speaker->id }}</td>
                                <td class="px-4 py-3">{{ $speaker->name}}</td>
